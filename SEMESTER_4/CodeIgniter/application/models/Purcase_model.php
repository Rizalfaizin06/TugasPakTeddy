...
class Purcase_model extends CI_Model
{
    ...
    public function get_new_purcase($rowno, $rowperpage, $PURCHID = "", $NAME = "", $TOTAL = "")
    {
        ...
        $this->db->like('PT.PURCHID', $PURCHID);
        $this->db->like('NAME', $NAME);
        if (!empty($TOTAL)) {
            $this->db->having('TOTAL', $TOTAL);
    ...
    //     return true;
    // }
    public function update_purcase($DATA_POST)
    {
        ...
        try {
            ...
            $this->db->set('PURCHSTATUS', $DATA_POST['PURCHSTATUS']);

            $this->db->where('PURCHID', $DATA_POST['PURCHID']);
            $this->db->update('purchtable');


            foreach ($DATA_POST as $key => $value) {
                if (strpos($key, 'PURCHRECEIVENOW') !== false) {
                    // Ambil 6 karakter pertama dari $key sebagai INVENTTRANSID
                    $inventTransID = substr($key, 0, 6);

                    // Update data pada tabel 'purchline' menggunakan INVENTTRANSID
                    $this->db->set('PURCHRECEIVEDNOW', $value);
                    $this->db->where('INVENTTRANSID', $inventTransID);
                    $this->db->update('purchline');
                }
            }
    ...
    public function get_purcase_id($PURCHID)
    {
        $this->db->select('*');
        $this->db->from('purchline AS PL');
        $this->db->join('purchtable AS PT', 'PL.PURCHID = PT.PURCHID');
        $this->db->join('vendtable AS V', 'PT.INVOICEACCOUNT = V.ACCOUNTNUM');
        $this->db->join('inventtable AS I', 'PL.ITEMID = I.ITEMID');
        $this->db->where('PT.PURCHID', $PURCHID);
        $result = $this->db->get();
    ...
    public function get_new_purcase_count($PURCHID = "", $NAME = "", $ITEMNAME = "")
    {
        $this->db->select('PT.PURCHID, NAME, SUM(LINEAMOUNT) AS TOTAL');
        $this->db->from('purchtable AS PT');
        $this->db->join('purchline AS PL', 'PL.PURCHID = PT.PURCHID');
